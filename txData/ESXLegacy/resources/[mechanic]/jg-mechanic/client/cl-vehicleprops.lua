...
---@param vehicle any
---@param props any
local function setStatebagsFromProps(vehicle, props)
...
function getVehicleProperties(vehicle, withStatebags)
  ...
  for i = 1, 15 do
    if DoesExtraExist(vehicle, i) then
      extras[i] = IsVehicleExtraTurnedOn(vehicle, i) == 1
    end
  end
...
function setVehicleProperties(vehicle, props, withStatebags)
  ...
  if props.extras then
    for id, enabled in pairs(props.extras) do
      SetVehicleExtra(vehicle, tonumber(id) --[[@as number]], enabled and 0 or 1 --[[@as boolean]]) 
    end
  end
